...
test.describe('Projects Management', () => {
  ...
  test('should add SEO fields to project', async ({ authenticatedPage }) => {
    const projectsPage = new ProjectsPage(authenticatedPage);
    ...
    // Fill required fields
    await projectsPage.getTitleInput().fill(generateRandomTitle('SEO Test'));
    await projectsPage.getDescriptionInput().fill('Test description for SEO');
    ...
    await authenticatedPage.locator('text=SEO Settings').scrollIntoViewIfNeeded();

    // Click generate button
    await authenticatedPage.locator('button:has-text("Generate from Title")').click();

    // Verify meta title is auto-generated
    const metaTitleInput = authenticatedPage.locator('#metaTitle');
    await expect(metaTitleInput).toHaveValue(/SEO Test.*- Starwood Interiors/);

    // Fill meta keywords
    await authenticatedPage.locator('#metaKeywords').fill('test, seo, project');

    // Verify field is filled
    await expect(authenticatedPage.locator('#metaKeywords')).toHaveValue('test, seo, project');
  });
